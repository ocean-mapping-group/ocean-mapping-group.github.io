...
## Output Files
*   `<outfile_name>`: A PostScript metafile (`.meta`) visualizing the time-series data.
*   `<infile_prefix>.tide.raster`: A binary JHC `.r4` array file (`.r4`) containing the rasterized plot (if `-raster` is used).
...
*   `j_attitude.h`: For attitude data structures.

## Notes
`plotTides` is an indispensable tool for quality control of tidal and motion data, allowing for visual inspection, statistical analysis, and correlation with multibeam survey periods. It helps in validating the accuracy of corrections applied and in identifying data gaps or anomalous trends. The extensive options for customization make it a flexible component in hydrographic data analysis workflows.