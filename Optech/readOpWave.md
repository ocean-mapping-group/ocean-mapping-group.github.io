...
*   **`noraman` flag:** The `-noraman` option's description in `USAGE` and its actual implementation logic in the provided code snippet (`if (noraman) { error(...) }`) appear contradictory or incomplete. It seems designed to prevent processing if RAMAN data *exists* while this flag is set, leading to an error.
```
```