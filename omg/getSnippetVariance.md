...
## How It Works
1.  **Initialization:** Initializes sum, sum_squares, and count arrays for statistical accumulation. Sets debugging flags.
...
5.  **Profile Iteration:** Loops through each profile (ping) in the merged file:
    ...
    *   **Grazing Angle and Ensonified Area Calculation:** Calls `get_dtm_grazing_angles` (to estimate grazing angles using the DTM) and `calc_ensonified_area` (to calculate ensonified areas).
    *   **Snippet Calibration:** For each beam with valid data:
        ...
        *   Calls `calibrate_reson` to apply calibration to these `IScans`, producing `Scans` (calibrated snippet intensities). This function uses grazing angles, ensonified areas, and profile/beam data.
