...
| `-roll_offset <val>` | Apply a constant roll offset (in degrees) to the data. Overrides `-apply_roll`. |
...
| `-pitch_offset <val>` | Apply a constant pitch offset (in degrees) to the data. Overrides `-apply_pitch`. |
...
5.  **In-Place Modification:** The modified beam records and profile headers are written back to the input merged files.
