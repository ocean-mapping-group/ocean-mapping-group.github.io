...
It expects both input files to have an identical set of sectors (defined by parameters like `sector_id`, `center_frequency`, `signal_length`, `signal_bandwidth`). It calculates the point-by-point difference in dB values for each angle bin.
...
## How It Works
1.  **Input Reading:** Reads the two input multi-sector beam pattern files (`-first` and `-second`) using `load_MS_bp`. This function populates `First_BP`/`Second_BP` (the actual beam pattern dB values) and `First_Params`/`Second_Params` (the TX sector parameters).
2.  **Sector Comparison:**
...
5.  **Output:** Writes the calculated differences (`Diff_BP`) along with the sector parameters (`First_Params`) to the specified output file using `dump_MS_bp`. The output file is a multi-sector beam pattern file, where the values represent the dB difference.
