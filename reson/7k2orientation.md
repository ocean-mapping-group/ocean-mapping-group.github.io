...
## Notes
`7k2orientation` is a critical tool for extracting motion and heading data from Reson 7k files into a standardized format. This data is essential for accurate motion compensation and georeferencing in subsequent multibeam sonar processing workflows. The tool supports multiple types of orientation records present in the 7k format, and creates separate output files for different data streams. The tool creates new files without modifying the original `.s7k` input.