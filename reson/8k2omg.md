...
## Notes
`8k2omg` provides a crucial bridge for older Reson SeaBat 8000 series data, allowing it to be integrated and processed within the modern OMG-HDCS framework. It handles the complex conversion from the proprietary binary format, extracting bathymetry and snippet data. The tool creates new files without modifying the original `.81x` input. The `-ER` option provides a specific adjustment for Extended Range projectors.