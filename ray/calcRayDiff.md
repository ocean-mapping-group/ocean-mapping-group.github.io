...
4.  **SVP Interpolation (`jWC_interp_zv()`):**
    *   Interprets the raw SVP data within `wc1` and `wc2` into a finely layered model (e.g., constant velocity layers). This prepares the water column for ray tracing.
...
*   `j_watercolumn.h`: For `watercolumn` structure and ray tracing functions (`jWC_init`, `jWC_interp_zv`, `jWC_trace`, `jWC_resample_timeseries`, `jWC_interp_XZ`, `jWC_free`).
*   `j_farray.h`: For `farray` structure (a generic 2D floating-point array structure with associated metadata).
