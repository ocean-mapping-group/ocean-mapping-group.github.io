...
## Notes
The lookup tables generated by `buildray` are essential for correcting multibeam sonar data for refraction effects. They provide a quick and accurate way to convert apparent sonar measurements (time and angle) into true depths and across-track positions. The tool creates new files, preserving the original SVP data, and allows for customization of the ray tracing parameters.