...
## Output Files
*   A PostScript metafile (`plot.meta` or specified by `-outfile`) containing the SVP plots.
*   `svp.coords`: ASCII file listing SVP locations.
*   `svp.r4`: JHC `.r4` array file containing SVP data (if `-array` is used).
*   `<svpfilename>.jwc`: JWC format SVP file (if `-outasJWC` is used).
*   `<svpfilename>.raster`: Raster image file (if `-raster` is used).
