...
## Description
`glboxfilt` is a utility designed to apply a box filter to JCU GLORIA format merged sidescan files. It processes the combined port and starboard sidescan data (`all_data`) using a sliding window filter to either smooth the imagery (low-pass) or extract high-frequency components (high-pass). This tool is similar in principle to `destripe.c` and `destripe_hack.c` but operates on the entire data swath simultaneously.

## Usage
```bash
glboxfilt [-filtlen <length>] [-filtwidth <width>] [-low | -high] [-skip <count>] <infile_prefix>
```
...
## How It Works
1.  **Initialization:** The tool initializes an array (`filter_array`) to hold `filtwidth` number of `gloria_all_rec` records, which act as a sliding window for the filter. It also initializes a sum array (`all`) and temporary arrays (`centre`, `high`) for processing, with a size of `GLORIA_SIDESIZE * 2`.
...
3.  **Sliding Window and Summation:**
    ...
    *   For each record in the window, it calculates the sum of pixel intensities (`all[k]`) for each across-track position `k` across the entire swath (`GLORIA_SIDESIZE * 2`).
4.  **Filtering (Main Loop):** For each record `j` in the input file:
    *   **Calculate Filtered Output:**
        ...
        *   If `high_flag` is set: The output pixel value is the difference between the central pixel's value and the average value within the filter window. This effectively extracts the high-frequency components.
    ...
    *   **Populate Output Record:** The filtered data is copied into a new `gloria_all_rec` record (`out_rec`), specifically to `out_rec.all_data`. The header information is copied from the central record of the window.
