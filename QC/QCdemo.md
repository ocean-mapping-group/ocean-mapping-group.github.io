...
5.  **Output (`synthetic.merged`):** If `-write_out` is specified, it creates a `synthetic.merged` file, dumps a summary header, and then writes `OMG_HDCS_profile_header` and `OMG_HDCS_beam` records for each simulated ping.

## Output
*   An interactive X-windows graphical interface.
*   Optionally, a `synthetic.merged` file containing simulated multibeam data.
*   Optionally, `history.grid` for dumping grid data.
```
```
---
layout: default
